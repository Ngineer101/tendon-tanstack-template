...
import { testMcpServerConnection } from "#/lib/mcp/core.server";

function extractIdFromPath(request: Request): string {
  const url = new URL(request.url);
  const match = url.pathname.match(/\/api\/mcp\/servers\/([^/]+)\/test$/);
  if (!match?.[1]) {
    ...
    handlers: {
      POST: authenticatedApiHandler<McpEnv>(
        async ({ env, request, user }) => {
          ...
          const result = await testMcpServerConnection(env, user.id, id);
